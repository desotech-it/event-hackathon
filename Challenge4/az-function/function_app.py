...
from model_houseloanterms import model_houseloanterms
...
@app.blob_trigger(arg_name="myblob", path="data/house/{name}",
                  connection="STORAGE_CONNECTION_STRING")
@app.cosmos_db_output(arg_name="outputDocument",
                      database_name="ContosoDB",
                      container_name="HouseLoanTerms",
                      connection="COSMOS_CONNECTION_STRING")
def ProcessHouseLoanTerms(myblob: func.InputStream, outputDocument: func.Out[func.Document]):
    """
    Triggered when a new blob is added to the 'house' container.
    Processes the blob and saves the results to Cosmos DB.
    
    Parameters:
    myblob (func.InputStream): The input blob stream.
    outputDocument (func.Out[func.Document]): The output document for Cosmos DB.
    """
    container_name = 'data/house'
    blob_service_client = get_blob_service_client()
    ...
    if file_name.endswith('.json'):
        ...
        # Model the house loan terms data
        document = model_houseloanterms(data)
